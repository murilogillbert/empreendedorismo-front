...
import { ChevronLeft, Save, Trash2, Beef, Wheat, Milk, Cookie, Egg, Soup, Check, LayoutGrid, Package } from 'lucide-react';
...
export const EditMenuItem: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (!managerActiveRestaurantId || !id) return;
            try {
                const [itemData, ingData, catData, menuData] = await Promise.all([
                    managerService.getMenuItem(managerActiveRestaurantId, Number(id)),
                ...
                reset({
                    ...
                    id_categoria: itemData.id_categoria,
                    ativo: itemData.ativo,
                    isCombo: false, // Default for now
                    ingredients: [], // To be fetched from ficha técnica if needed
                    comboItems: [],
                    allergens: []
                });
        ...
        fetchData();
    }, [managerActiveRestaurantId, id, reset]);
    ...
    const onSubmit = async (data: FormData) => {
        ...
        setSubmitting(true);
        try {
            await managerService.updateMenuItem(managerActiveRestaurantId, Number(id), {
                nome: data.name,
                descricao: data.description,
                preco: data.price,
                id_categoria: data.id_categoria,
                ativo: data.ativo,
                // ingredients: data.ingredients, // Update relations if supported
    ...
    const handleDelete = async () => {
        ...
        if (window.confirm('Excluir este item permanentemente?')) {
            try {
                await managerService.deleteMenuItem(managerActiveRestaurantId, Number(id));
                navigate('/manager/menu');
            } catch (error) {
    ...
    const toggleSelection = (field: 'ingredients' | 'comboItems', idValue: number) => {
        const current = watch(field);
        if (current.includes(idValue)) {
            setValue(field, current.filter((i: number) => i !== idValue));
        } else {
            setValue(field, [...current, idValue]);
        }
    };
            ...
            {loading ? (
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <Card className="flex items-center justify-between p-4 bg-primary/5 border-primary/10">
                            <div className="flex items-center gap-3">
                                <div className={cn(
                                    "p-1.5 rounded-lg",
                                    watch('ativo') ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"
                                )}>
                            ...
                            <button
                                ...
                                className={cn(
                                    ...
                                    watch('ativo') ? "bg-primary" : "bg-gray-200"
                                )}
                            >
                                <div className={cn(
                                    "absolute top-0.5 size-4 bg-white rounded-full transition-all",
                                    watch('ativo') ? "right-0.5" : "left-0.5"
                                )}></div>
                            </button>
                        </Card>
                    ...
                    <button
                        type="submit"
                        disabled={submitting}
                        className="w-full bg-black text-white font-bold py-4 rounded-xl shadow-lg hover:opacity-90 transition-all flex items-center justify-center gap-2 active:scale-[0.98] disabled:opacity-50"
                    >
                        {submitting ? (
